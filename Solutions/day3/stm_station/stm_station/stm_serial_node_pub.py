...
class SerialPublisherNode(Node):
    def __init__(self):
        ...
        self.port = '/dev/ttyACM0'  # Update with the correct serial port
        ...
        # 5. Create a publisher to publish the received data from the serial port
        ...
        if self.serial_port.in_waiting > 0:
            try:
                ...
                if len(float_values) == 8:
                    ...
                    # Convert each string value to float, stripping any surrounding whitespace
                    float32_multi_array_msg.data = [float(val.strip()) for val in float_values]

                    # Publish the message
                    self.stm_state_publisher.publish(float32_multi_array_msg)

                    self.get_logger().info(f'Published data: {float32_multi_array_msg.data}')
                else:
                    self.get_logger().warn(f'Unexpected number of data points: {len(float_values)} instead of 8')
